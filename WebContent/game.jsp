...
<head>
	<meta charset="UTF-8">
	<title>Gomoku Game Board</title>
</head>
<body onload="startLoad()" style="padding:0px;margin:0 auto">
...
var winner = ''; //赢家初始化为空
var img_b = new Image();
img_b.src = "image/black.png";//黑棋图片
var img_w = new Image();
img_w.src = "image/white.png";//白棋图片
var step=225;//总步数
    var moves = 0;
...
function chess(color, x, y) {
  ...
  context.beginPath();
  context.drawImage(img_b, x * 40 + 20, y * 40 + 80);
  context.closePath();
  context.fill();
  if (color == "white") {
...
function success(a, b, c, d, temp, count) {
  if (count == 5) { //因为落子点重复计算了一次
    ...
    setCookie("winner", winner);
    alert(winner);
    //function1 post result(userID,time,moves,winOrLose)
    //function2 post result(userID, blank array.toString)
